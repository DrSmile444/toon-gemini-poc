/* eslint-disable sonarjs/slow-regex,@typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-assignment,no-continue,no-plusplus,no-use-before-define,@typescript-eslint/no-require-imports,sonarjs/updated-loop-counter,global-require,@typescript-eslint/no-unsafe-call,unicorn/no-process-exit,sonarjs/no-alphabetical-sort,unicorn/no-array-sort,security/detect-non-literal-fs-filename,security/detect-object-injection,security/detect-unsafe-regex */
...
interface UninstalledDependencies {
  referencedTopLevel: Set<string>;
  uninstalled: string[];
}

async function findUninstalledDeps(root: string): Promise<UninstalledDependencies> {
